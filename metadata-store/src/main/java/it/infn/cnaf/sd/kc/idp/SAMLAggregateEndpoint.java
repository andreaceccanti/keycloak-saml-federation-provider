...
public class SAMLAggregateEndpoint {
  ...
  public SAMLAggregateEndpoint(RealmModel realm, SAMLAggregateIdentityProvider provider,
      ...
      DestinationValidator destinationValidator, SAMLIdpDescriptor idpDescriptor) {
    this.realm = realm;
    this.config = config;
  ...
  private URI getURIWithoutParameters(String url) throws URISyntaxException {
    URI uri = new URI(url);
  ...
  protected abstract class Binding {
    ...
    protected abstract void verifySignature(String key, SAMLDocumentHolder documentHolder)
    ...
    protected KeyLocator getIDPKeyLocator() {
      ...
      for (String signingCertificate : config.getSigningCertificates()) {
        ...
        try {
          cert = XMLSignatureUtil
            .getX509CertificateFromKeyInfoString(signingCertificate.replaceAll("\\s", ""));
          cert.checkValidity();
    ...
    protected Response handleSamlRequest(String samlRequest, String relayState) {
      ...
      if (isDestinationRequired() && requestAbstractType.getDestination() == null
      ...
      if (!destinationValidator.validate(getExpectedDestination(config.getAlias(), null),
    ...
    protected Response logoutRequest(LogoutRequestType request, String relayState) {
      ...
      if (request.getSessionIndex() == null || request.getSessionIndex().isEmpty()) {
        ...
        session.sessions()
          ...
          .collect(Collectors.toList()) // collect to avoid concurrent modification as
                                        // backchannelLogout removes the user sessions.
        ...
        for (String sessionIndex : request.getSessionIndex()) {
          ...
          UserSessionModel userSession =
              session.sessions().getUserSessionByBrokerSessionId(realm, brokerSessionId);
          if (userSession != null) {
            ...
            for (Iterator<SamlAuthenticationPreprocessor> it =
                SamlSessionUtils.getSamlAuthenticationPreprocessorIterator(session); it
                  .hasNext();) {
      ...
      JaxrsSAML2BindingBuilder binding =
          new JaxrsSAML2BindingBuilder(session).relayState(relayState);
      ...
      if (config.isWantAuthnRequestsSigned()) {
        ...
        binding.signWith(keyName, keys.getPrivateKey(), keys.getPublicKey(), keys.getCertificate())
          ...
          .signDocument();
        if (!postBinding && config.isAddExtensionsElementWithKeyInfo()) { // Only include extension
                                                                          // if REDIRECT binding and
                                                                          // signing whole SAML
                                                                          // protocol message
      ...
      try {
        if (postBinding) {
          return binding.postBinding(builder.buildDocument())
            .response(config.getSingleLogoutServiceUrl());
        } else {
          return binding.redirectBinding(builder.buildDocument())
            .response(config.getSingleLogoutServiceUrl());
        }
    ...
    protected Response handleLoginResponse(String samlResponse, SAMLDocumentHolder holder,
      ...
      try {
        ...
        final boolean inResponseToValidationSuccess =
            validateInResponseToAttribute(responseType, expectedRequestId);
        ...
        ConditionsValidator.Builder cvb =
            new ConditionsValidator.Builder(assertion.getID(), assertion.getConditions(),
                destinationValidator).clockSkewInMillis(1000 * config.getAllowedClockSkew());
        try {
          String issuerURL = getEntityId(session.getContext().getUri(), realm);
     ...
     * {@code clientUrlName}, creates a fresh authentication session for that client and returns a
     * {@link AuthenticationSessionModel} object with that session. Otherwise returns "client not
     * found" response.
     *
     * @param clientUrlName
    ...
    public Response handleSamlResponse(String samlResponse, String relayState, String clientId) {
      ...
      // validate destination
      if (isDestinationRequired() && statusResponse.getDestination() == null
          && containsUnencryptedSignature(holder)) {
        event.event(EventType.IDENTITY_PROVIDER_RESPONSE);
        event.detail(Details.REASON, Errors.MISSING_REQUIRED_DESTINATION);
      ...
      try {
        destinationURI = getURIWithoutParameters(statusResponse.getDestination());
    ...
    private String getExpectedDestination(String providerAlias, String clientId) {
      ...
      return Urls
        .identityProviderAuthnResponse(session.getContext().getUri().getBaseUri(), providerAlias,
            realm.getName())
        .toString();
    }
  }
  ...
  protected class PostBinding extends Binding {
    ...
    @Override
    protected void verifySignature(String key, SAMLDocumentHolder documentHolder)
        throws VerificationException {
      ...
      SamlProtocolUtils.verifyDocumentSignature(documentHolder.getSamlDocument(),
          getIDPKeyLocator());
    }
  ...
  protected class RedirectBinding extends Binding {
    ...
    protected boolean containsUnencryptedSignature(SAMLDocumentHolder documentHolder) {
      MultivaluedMap<String, String> encodedParams =
          session.getContext().getUri().getQueryParameters(false);
  ...
  private String getFirstMatchingAttribute(AssertionType assertion,
  ...
  private boolean validateInResponseToAttribute(ResponseType responseType,
    ...
    // If present, Assertion > Subject > Confirmation > SubjectConfirmationData > InResponseTo must
    // also be validated
    ...
    if (subjectElement != null) {
      if (subjectElement.getConfirmation() != null && !subjectElement.getConfirmation().isEmpty()) {
        SubjectConfirmationType subjectConfirmationElement =
            subjectElement.getConfirmation().get(0);
        ...
        if (subjectConfirmationElement != null) {
          ...
          if (subjectConfirmationDataElement != null) {
            if (subjectConfirmationDataElement.getInResponseTo() != null) {
              // 3) Assertion > Subject > Confirmation > SubjectConfirmationData > InResponseTo is
              // empty
