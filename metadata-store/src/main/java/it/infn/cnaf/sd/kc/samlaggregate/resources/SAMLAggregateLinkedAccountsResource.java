...
public class SAMLAggregateLinkedAccountsResource {

  private static final Logger logger = Logger.getLogger(SAMLAggregateLinkedAccountsResource.class);
  ...
  public SAMLAggregateLinkedAccountsResource(KeycloakSession session, HttpRequest request,
      ClientModel client, Auth auth, EventBuilder event, UserModel user) {
    ...
    this.federatedIdentitiesService = session.getProvider(SAMLAggregateFederatedIdentityServiceProvider.class);
  }
  ...
  public Response linkedAccounts() {
    ...
    SortedSet<LinkedAccountRepresentation> linkedAccounts =
        getLinkedAccounts(this.session, this.realm, this.user);
  ...
  public SortedSet<LinkedAccountRepresentation> getLinkedAccounts(KeycloakSession session,
      RealmModel realm, UserModel user) {

    Set<String> socialIds = findSocialIds();

    SortedSet<LinkedAccountRepresentation> linkedAccounts = realm.getIdentityProvidersStream()
  ...
  private LinkedAccountRepresentation toLinkedAccountRepresentation(SamlAggregateFederatedIdentityDTO fir) {
    ...
    rep.setDisplayName(fir.getIdentityProviderAlias());
  ...
  private LinkedAccountRepresentation toLinkedAccountRepresentation(IdentityProviderModel provider,
      Set<String> socialIds, Stream<FederatedIdentityModel> identities) {
  ...
  public Response buildLinkedAccountURI(@PathParam("providerId") String providerId,
    ...
    try {
      ...
      URI linkUri = Urls.identityProviderLinkRequest(
          this.session.getContext().getUri().getBaseUri(), providerId, realm.getName());
      linkUri = UriBuilder.fromUri(linkUri)
        .queryParam("nonce", nonce)
        .queryParam("hash", hash)
        // need to use "account-console" client because IdentityBrokerService
        // authenticates user using cookies
        // the regular "account" client is used only for REST calls therefore cookies
  ...
  public Response removeLinkedAccount(@PathParam("providerId") String providerId) {
    ...
    logger.debugv("Social provider {0} removed successfully from user {1}", providerId,
        user.getUsername());
  ...
  private boolean isPasswordSet() {
    return session.userCredentialManager()
      .isConfiguredFor(realm, user, PasswordCredentialModel.TYPE);
  }
